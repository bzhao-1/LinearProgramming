# Ben Zhao. This code was written in collaboration with Khizar Qureshi and Nathan Streiff
#  Adapted from starter script provided by Prof Rob Thompson for solving the convex (quadratic)
...
@objective(model, Min, transpose(x) * transpose(Q) * Q * x - sum(pdp[i] * x[i] for i in 1:n));

# Define your constraint
@constraint(model,c1, sum(x[i] for i in 1:n) == 1);
